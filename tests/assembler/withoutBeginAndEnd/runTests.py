...
tests = sorted([file for file in os.listdir("in") if file.endswith(".pre")], 
...
for testNum in range(len(tests)):
    ...
    if(result.returncode != 0):
        print(f"{BOLD}{RED}Failed{RESET}, with statuscode: {result.returncode}, Output: {result.stderr}")
        continue

    test_output_name = tests[testNum].replace(".pre", ".obj")
    ...
    if diff.returncode == 0:
        ...
        print(f"{BOLD}{RED}Failed{RESET}. Output: {diff.stdout}")
